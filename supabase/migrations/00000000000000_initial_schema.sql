...
CREATE TABLE master_zmanim_registry (
    ...
    display_offset VARCHAR(20) DEFAULT 'day_of',
    ...
    CONSTRAINT chk_display_offset CHECK (display_offset IN ('day_before', 'day_of', 'day_after'))
);
...
CREATE INDEX idx_master_registry_event_category ON master_zmanim_registry(event_category) WHERE event_category IS NOT NULL;
...
COMMENT ON COLUMN master_zmanim_registry.event_category IS 'Category for event-specific zmanim. NULL means everyday zman. Values: candles, havdalah, yom_kippur, fast_day, tisha_bav, pesach';
