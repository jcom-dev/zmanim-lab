...
# Test Database & Cache (local containers)
...
NEXT_PUBLIC_SUPABASE_URL=${TEST_DATABASE_URL}
...
ENVEOF
    ...
    echo "  - Database: ${TEST_POSTGRES_HOST:-localhost}:${TEST_POSTGRES_PORT:-5433}"
    echo "  - Redis: ${TEST_REDIS_HOST:-localhost}:${TEST_REDIS_PORT:-6380}"
    echo "  - Clerk: ${CLERK_TEST_SECRET_KEY:+TEST instance}${CLERK_TEST_SECRET_KEY:-⚠️  Using LIVE Clerk (set CLERK_TEST_SECRET_KEY)}"
    echo "  - MailSlurp: ${MAILSLURP_API_KEY:+configured}${MAILSLURP_API_KEY:-⚠️  not configured}"
else
    print_warning "TEST_DATABASE_URL not set - E2E tests will use production database"
...
echo "  - Email: ${MAILSLURP_API_KEY:+MailSlurp}${MAILSLURP_API_KEY:-not configured}"
echo ""
...
echo "  - PostgreSQL (test): localhost:${POSTGRES_PORT:-5433}"
echo "  - Redis (test): localhost:${REDIS_PORT:-6380}"
echo ""
echo "🧪 Testing:"
